        ...
        if(BeginTable("##ScrollingRegion", 3, flags, {size.x - cw, 0.f}))
        {
            ...
            while(clipper.Step())
            {
                for(int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
                {
                    ...
                    else if(app.profiler_selected_hotspot >= 0)
                    {
                        auto color = i % 2 ?
                            IM_COL32(65, 0, 0, 255) :
                            IM_COL32(50, 0, 0, 255);
            ...
            if(do_scroll >= 0)
            {
                ...
                if(!app.scroll_addr_to_top) index -= num_display / 2;
                SetScrollY(clipper.ItemsHeight * index);
                app.scroll_addr_to_top = false;
        ...
        if(app.emulator.program_state.elf)
        {
            ...
            for(auto const& kv : app.emulator.program_state.elf->text_symbols)
            {
                ...
                if(mp.x >= ra.x && mp.y >= ra.y && mp.x < rb.x && mp.y < rb.y)
                {
                    ...
                    if(IsMouseClicked(0))
                    {
                        app.disassembly_scroll_addr = sym.addr;
                        app.scroll_addr_to_top = true;
                    }
                }
            }
