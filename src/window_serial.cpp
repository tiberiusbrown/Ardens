...
void window_serial(bool& open)
{
    ...
    bool scroll = false;
    {
        size_t t = app.emulator.core_state.cpu.serial_bytes.size();
        scroll = (t > prev_size);
        prev_size = t;
    }
    ...
    if(Begin("Serial Monitor", &open) && app.emulator.core_state.cpu.decoded)
    {
        auto& buf = app.emulator.core_state.cpu.serial_bytes;
        if(Button("Clear"))
            buf.clear();
