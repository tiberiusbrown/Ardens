...
namespace
{
constexpr uint8_t TWINT = reg::bit::TWCR::TWINT;
constexpr uint8_t TWEA  = reg::bit::TWCR::TWEA;
constexpr uint8_t TWSTA = reg::bit::TWCR::TWSTA;
constexpr uint8_t TWSTO = reg::bit::TWCR::TWSTO;
constexpr uint8_t TWWC  = reg::bit::TWCR::TWWC;
constexpr uint8_t TWEN  = reg::bit::TWCR::TWEN;
constexpr uint8_t TWIE  = reg::bit::TWCR::TWIE;
constexpr uint8_t PRTWI = reg::bit::PRR0::PRTWI;
}
...
ARDENS_FORCEINLINE void atmega32u4_t::twi_finish_status(uint8_t status)
{
    ...
    data[reg::addr::TWSR] =
        uint8_t((data[reg::addr::TWSR] & 0x03) | twi_status);
    data[reg::addr::TWCR] |= TWINT;
...
{
    uint32_t prescaler = 1u << ((data[reg::addr::TWSR] & 0x03) * 2u);
    uint32_t scl_cycles = 16u + 2u * data[reg::addr::TWBR] * prescaler;
...
void atmega32u4_t::update_twi()
{
    ...
    case TWI_PENDING_STOP:
        ...
        twi_pull_scl_low = false;
        twi_pull_sda_low = false;
        data[reg::addr::TWCR] &= uint8_t(~TWSTO);
        data[reg::addr::TWSR] =
            uint8_t((data[reg::addr::TWSR] & 0x03) | 0xf8);
        twi_status = 0xf8;
        break;
    ...
    case TWI_PENDING_READ:
    {
        ...
        twi_finish_status((data[reg::addr::TWCR] & TWEA) ? 0x50 : 0x58);
        break;
    }
...
{
    if((cpu.data[reg::addr::PRR0] & PRTWI) ||
        !(cpu.data[reg::addr::TWCR] & TWEN))
        return;

    uint8_t twcr = cpu.data[reg::addr::TWCR];
    ...
    case 0x08:
    case 0x10:
        cpu.twi_address = cpu.data[reg::addr::TWDR] >> 1;
        cpu.twi_reading = (cpu.data[reg::addr::TWDR] & 1) != 0;
    ...
    case 0x48:
    case 0x58:
        cpu.data[reg::addr::TWSR] =
            uint8_t((cpu.data[reg::addr::TWSR] & 0x03) | 0xf8);
        cpu.twi_status = 0xf8;
        break;
...
{
    assert(ptr == reg::addr::TWCR);
    cpu.update_twi();

    uint8_t old = cpu.data[reg::addr::TWCR];
...
void atmega32u4_t::twi_handle_st_twdr(
    atmega32u4_t& cpu, uint16_t ptr, uint8_t x)
{
    assert(ptr == reg::addr::TWDR);
    cpu.update_twi();
    if(!(cpu.data[reg::addr::TWCR] & TWINT))
    {
        cpu.data[reg::addr::TWCR] |= TWWC;
        return;
    }
    cpu.data[ptr] = x;
...
{
    bool twi_enabled =
        (data[reg::addr::TWCR] & TWEN) != 0 &&
        (data[reg::addr::PRR0] & PRTWI) == 0;
...
uint8_t atmega32u4_t::twi_handle_ld_pind(atmega32u4_t& cpu, uint16_t ptr)
{
    ...
    auto apply_input_level = [&](uint8_t bit, bool high)
    {
        ...
        if(cpu.data[reg::addr::DDRD] & mask)
            return;
        if(high)
            value |= mask;
        else
...
{
    if((data[reg::addr::PRR0] & PRTWI) ||
        !(data[reg::addr::TWCR] & TWEN))
        return false;

    uint8_t own = data[reg::addr::TWAR] >> 1;
    uint8_t mask = data[reg::addr::TWAMR] >> 1;
    bool own_match = ((address ^ own) & uint8_t(~mask)) == 0;
    bool general_match =
        general_call && address == 0 && (data[reg::addr::TWAR] & reg::bit::TWAR::TWGCE);
    if(general_call && address == 0)
        return general_match || ((data[reg::addr::TWCR] & TWEA) && own_match);
    return own_match || general_match;
}

bool atmega32u4_t::twi_slave_can_address(uint8_t address, bool general_call) const
{
    if(!(data[reg::addr::TWCR] & TWEA))
        return false;
...
bool atmega32u4_t::twi_slave_write(uint8_t data)
{
    ...
    this->data[reg::addr::TWDR] = data;
    bool ack = (this->data[reg::addr::TWCR] & TWEA) != 0;
    if(twi_general_call)
...
uint8_t atmega32u4_t::twi_slave_read(bool master_ack)
{
    ...
    uint8_t data = this->data[reg::addr::TWDR];
    if(master_ack)
        twi_finish_status(0xb8);
    else
        twi_finish_status((this->data[reg::addr::TWCR] & TWEA) ? 0xc8 : 0xc0);
    return data;
}
